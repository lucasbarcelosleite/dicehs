 ...
 * @package Joomla / Tradu��o idioma pt-br 29/08/2006 /por: Lena - www.facilhost.com.br
...
function mosLoadAdminModule( $name, $params=NULL ) {
	...
	$name = str_replace( '\\', '', $name );
	$path = LIVE_ADMIN."/modules/mod_$name.php";
	if (file_exists( $path )) {
		require $path;
	}
}
...
function mosLoadCustomModule( &$module, &$params ) {
	...
	if ( $rssurl ) {
		if (!is_writable( $cachePath )) {
			echo '<tr>';
			echo '<td>Por favor, altere as permiss�es do diret�rio de cache para ser edit�vel.</td>';
			echo '</tr>';
		} else {
...
function mosMainBody_AdminReturn() {
	return $GLOBALS['_MOS_OPTION']['buffer'];
}
...
function josSecurityCheck($width='95%') {
	...
	if ( RG_EMULATION != 0 ) {
		$wrongSettingsTexts[] = 'Configura��o Joomla no ficheiro globals.php - Emula��o Joomla do Registo de Globais (RG_EMULATION) est� `Ligado` em vez de `Desligado` <br /><span style="font-weight: normal; font-style: italic; color: #666;">Est� `Ligado` por omiss�o mas apenas por quest�es de compatibilidade com algumas extens�es.</span>';
	}
...
<p style="color: #CC0000;">As seguintes Configura��es de Seguran�a do
Servidor PHP n�o est�o otimizadas para <strong>SEGURAN�A</strong> e �
recomendado que as altere:</p>
<ul
	style="margin: 0px; padding: 0px; padding-left: 15px; list-style: none;">
	<?php
	foreach ($wrongSettingsTexts as $txt) {
		?>
	<li
		style="min-height: 25px; padding-bottom: 5px; padding-left: 25px; color: red; font-weight: bold; background-image: url(../includes/js/ThemeOffice/warning.png); background-repeat: no-repeat; background-position: 0px 2px;">
		<?php
		echo $txt;
		?></li>
		<?php
...
<p style="color: #666;">Por favor, consulte o <a
	href="http://forum.joomla.org/index.php/topic,81058.0.html"
	target="_blank" style="color: blue; text-decoration: underline">artigo
oficial Joomla! sobre Seguran�a de Servidores</a> para mais informa��es.
</p>
</div>
	<?php
	}
...
<div style="font-weight: normal; color: #666;">A sua vers�o Joomla! [ <?php echo $versioninfo; ?>
] tem</div>
<div style="font-size: 13px; color: #CC0000;"><?php echo $age; ?> dias
de exist�ncia.</div>
<div><input class="button" name="Button3" type="submit"
	value="Verificar se existe nova vers�o"
