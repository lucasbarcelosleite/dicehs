...
class pChart
{
	...
	function clearScale()
	{
		...
		function drawLegend($XPos,$YPos,$DataDescription,$R,$G,$B,$Rs=-1,$Gs=-1,$Bs=-1,$Rt=0,$Gt=0,$Bt=0,$Border=TRUE)
		{
			...
			{
				if($Key == 'Serie1'){
					$YOffset = $this->FontSize-1;
				}
				$this->drawFilledRoundedRectangle($XPos+5,$YPos+$YOffset-5,$XPos+14,$YPos+$YOffset-5,5,$this->Palette[$ID]["R"],$this->Palette[$ID]["G"],$this->Palette[$ID]["B"]);
